...
class Settings:
    ...
    geminiApiKey: str = ""
    geminiModel: str = "gemini-2.0-flash-exp"
    easyThreshold: int = 5
        ...
        try:
            ...
            for field in required_fields:
                if field not in settings:
                    raise SettingsError(f"필수 설정 누락: {field}")
            
            # 값 범위 검사
            if "temperature" in settings:
                temp = float(settings["temperature"])
                if not 0.0 <= temp <= 1.0:
                    raise SettingsError("temperature는 0.0과 1.0 사이여야 합니다")
            ...
            for threshold in thresholds:
                if threshold in settings:
                    ...
                    if not 0 <= value <= 100:
                        raise SettingsError(f"{threshold}는 0과 100 사이여야 합니다")
            
        except ValueError as e:
            raise SettingsError(f"설정값 형식 오류: {str(e)}")
